...
## Prompter repository context

Prompter is the Discord documentation assistant, not a generic vertical-slice app. Read [the repository overview and configuration](../../README.md) and the **Commands** and introductory product facts in [root CLAUDE.md](../../CLAUDE.md). Its legacy sibling-rule/propagation fallback and build-every-task wording are retired: use this local corpus and proportional checks instead. Keys belong only in approved secret mechanisms, never tracked context or an unverified ignore path. Local facts may narrow shared behavior but cannot weaken security, exact effect authorization, or required gates.
