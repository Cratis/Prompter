...
public class and_the_product_is_unknown : Specification
{
    readonly GitHubOptions _options = new();
    string _repository = null!;

    void Because() => _repository = IssueRouting.RepositoryFor("orleans", _options);

    [Fact] void should_fall_back_to_the_default() => _repository.ShouldEqual(_options.DefaultRepository);
    [Fact] void should_not_consider_it_routed() => IssueRouting.IsRouted("orleans", _options).ShouldBeFalse();
}
